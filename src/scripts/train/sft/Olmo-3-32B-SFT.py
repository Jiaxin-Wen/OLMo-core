"""
This script can be used to launch an SFT run for the 32B model on Beaker or Slurm.
...
class SFTConfig(Config):
    ...
    def build(
        ...
        *,
        ...
        if cluster == "slurm":
            import os
            user_name = os.environ.get("USER", "slurm_user")
        else:
        ...
        # No RoPE scaling needed: 8192 is the native context length for stage1 pretraining
        ...
        config = SFTConfig(
            ...
            .with_callback(
                "checkpointer",
                CheckpointerCallback(
                    save_interval=1000, save_async=True
                ),
            )
            .with_callback(
                "wandb",
...
""",
    ...
    parser.add_argument(
        "--gpus_per_node", type=int, help="GPUs per node.", default=DEFAULT_GPUS_PER_NODE
    )
    parser.add_argument(
        "--no_save_tokenizer",
        action="store_true",
    ...
    if args.cmd == "dry_run":
        ...
        if config.launch is None:
            raise RuntimeError("Cannot launch on slurm cluster via this command")
